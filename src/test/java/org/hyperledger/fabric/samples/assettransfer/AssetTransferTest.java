...
public final class AssetTransferTest {
    ...
    class InvokeDuplicateAssetTransaction {
        ...
        public void whenAssetDoesNotExist() {
            ...
            Throwable thrown = catchThrowable(() -> {
                contract.DuplicateAsset(ctx, "asset1", "asset123", "Kamal");
            });
        ...
        @Test
        public void whenAssetCanBeDuplicated() {
            AssetTransfer contract = new AssetTransfer();
            Context ctx = mock(Context.class);
            ChaincodeStub stub = mock(ChaincodeStub.class);
            when(ctx.getStub()).thenReturn(stub);
            ...
            Asset asset = contract.DuplicateAsset(ctx, "asset1", "asset123", "Kamal");

            assertThat(asset).isEqualTo(new Asset("asset123", "blue", 5, "Kamal", 300));

        }

        @Test
        public void whenDuplicateAssetAlreadyExists() {
            AssetTransfer contract = new AssetTransfer();
            Context ctx = mock(Context.class);
            ChaincodeStub stub = mock(ChaincodeStub.class);
            when(ctx.getStub()).thenReturn(stub);
            when(stub.getStringState("asset1"))
                    .thenReturn("{ \"assetID\": \"asset1\", \"color\": \"blue\", \"size\": 5, \"owner\": \"Tomoko\", \"appraisedValue\": 300 }");
            when(stub.getStringState("asset123"))
                    .thenReturn("{ \"assetID\": \"asset123\", \"color\": \"blue\", \"size\": 5, \"owner\": \"Kamal\", \"appraisedValue\": 300 }");
            ...
            assertThat(thrown).isInstanceOf(ChaincodeException.class).hasNoCause()
                    .hasMessage("Asset asset123 already exists");
            assertThat(((ChaincodeException) thrown).getPayload()).isEqualTo("ASSET_ALREADY_EXISTS".getBytes());
        }
